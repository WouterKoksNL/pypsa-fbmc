...
def _read_generation_mix(case_dir: Path) -> pd.DataFrame:
	generation_mix_path = case_dir / "generation_mix.csv"
...
def _load_run_generation_mix(run_dirs: Iterable[Path]) -> pd.DataFrame:
	generation_mix_frames = [_read_generation_mix(run_dir) for run_dir in run_dirs]
	generation_mix_frames = [frame for frame in generation_mix_frames if not frame.empty]
	if not generation_mix_frames:
		return pd.DataFrame()
	combined_generation_mix = pd.concat(generation_mix_frames, axis=0)
