...
from example_networks.advanced_hybrid_check import create_advanced_hybrid_check
...
def create_case(case: str | Cases, load_case_flag=False, save_case_flag=True, **kwargs):
    if type(case) is str:
        case = Cases(case)
...
def alter_case_workflow(output: dict[str, Any], case_alteration_kwargs: dict[str, Any]):
    from example_networks.case_alteration import select_snapshots, apply_uc, remove_links
