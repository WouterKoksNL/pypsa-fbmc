...
def create_double_three_node_link_and_line_case():
    ...
    for sn in [1, 2]:
        ...
        nodal_net.add('Generator', f'gen_S{sn}A1', bus=f'S{sn}A1', p_nom=20, marginal_cost=400, carrier="Wind")
        nodal_net.add('Generator', f'gen_S{sn}B1', bus=f'S{sn}B1', p_nom=12, marginal_cost=100, carrier="CCGT")
        nodal_net.add('Generator', f'gen_S{sn}B2', bus=f'S{sn}B2', p_nom=20, marginal_cost=200, carrier="Oil")
        nodal_net.add('Load', f'load_S{sn}A1', bus=f'S{sn}A1', p_set=loads[sn])

    nodal_net.add('Bus', 'S1A1+')
    nodal_net.buses.loc['S1A1+', 'zone_name'] = 'S1A'
    nodal_net.add('Line', 'S1A1-S1A1+', bus0='S1A1', bus1='S1A1+', x=1, s_nom=1)
    nodal_net.add('Link', 'S2A1_S2A1+', bus0='S2A1', bus1='S1A1+', p_min_pu=-1, p_nom=5)
