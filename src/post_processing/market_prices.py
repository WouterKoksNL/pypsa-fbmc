...
def calculate_zonal_prices(
    zones,
    snapshots,
    z_ptdf_dict: dict[str],
    model: lp.Model,
    ...
    slack_zone_duals = model.constraints["Zone-definition"].dual.sel(
        Zone=sn_slack_zone_map.values
    )

    zonal_prices_full = pd.DataFrame(index=snapshots, columns=zones)

    for sub_network, z_ptdf in z_ptdf_dict.items():
        slack_zone_dual_ser = slack_zone_duals.sel(
            Zone=sn_slack_zone_map[sub_network]
        )  # timeseries
        ...
        cnec_upper_ram_dual = model.constraints[
        ...
        if f"CNEC-lower-RAM-subnet-{sub_network}" in model.constraints:
            cnec_lower_ram_dual = model.constraints[
                f"CNEC-lower-RAM-subnet-{sub_network}"
            ].dual
            zonal_price += (z_ptdf * cnec_lower_ram_dual).sum(dim="cnec")
        ...
        if "net_position_upper_limit_UA" in model.constraints:
            ...
            zonal_price.loc[upper_dual.snapshot, upper_dual.Zone] += upper_dual
